...
if(!class_exists('Themonic_Options') ){
    ...
    class Themonic_Options {
        ...
        protected $framework_url = 'http://www.themonic.com/';
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['last_tab'] = '0';
            $defaults['menu_icon'] = Themonic_OPTIONS_URL . '/img/menu_icon.png';
            $defaults['menu_title'] = __('Options', Themonic_TEXT_DOMAIN);
            $defaults['page_icon'] = 'icon-themes';
            $defaults['page_title'] = __('Options', Themonic_TEXT_DOMAIN);
            ...
            $defaults['footer_credit'] = sprintf(__('<span id="footer-thankyou">Crafted on <a href="%s" target="_blank">Themonic Framework</a> v%s</span>', Themonic_TEXT_DOMAIN), $this->framework_url, $this->framework_version);
            $defaults['help_tabs'] = array();
            $defaults['help_sidebar'] = __('', Themonic_TEXT_DOMAIN);
            ...
            if($this->args['page_type'] == 'submenu') {
                ...
                if(true === $this->args['allow_sub_menu']) {
                    ...
                    if(true === $this->args['show_import_export']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Import / Export', Themonic_TEXT_DOMAIN), 
                            __('Import / Export', Themonic_TEXT_DOMAIN), 
                            $this->args['page_cap'], 
                    ...
                    if(true === $this->args['dev_mode']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Dev Mode Info', Themonic_TEXT_DOMAIN), 
                            __('Dev Mode Info', Themonic_TEXT_DOMAIN), 
                            $this->args['page_cap'], 
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('themonic-opts-js', 'themonic_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', Themonic_TEXT_DOMAIN), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _options_page_html() {
			...
			echo '<a style="float:left; padding-right: 30px;" href="http://themonic.com" id="optionpanellogo" class="logo" target="_blank"><img src="'.$this->url.'img/logo.png" /></a>';
echo '<span style="float:left; margin-left: 70px;
    margin-top: 30px; color:#fff;" class="headtext">Unleash your Creativity with Iconic Themes</span>';
            ...
            echo ' &nbsp; ';
            submit_button(__('Reset to Defaults', Themonic_TEXT_DOMAIN), 'primary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('themonic-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="themonic-opts-imported">' . apply_filters('themonic-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', Themonic_TEXT_DOMAIN)) . '</div>';
                } else {
                    echo '<div id="themonic-opts-save">' . apply_filters('themonic-opts-saved-text-' . $this->args['opt_name'], __('<strong>Settings Saved!</strong>', Themonic_TEXT_DOMAIN)).'</div>';
                }
                delete_transient('themonic-opts-saved');
            }

            echo '<div id="themonic-opts-save-warn">' . apply_filters('themonic-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', Themonic_TEXT_DOMAIN)) . '</div>';
            echo '<div id="themonic-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', Themonic_TEXT_DOMAIN) . '</div>';
            echo '<div id="themonic-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', Themonic_TEXT_DOMAIN).'</div>';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="themonic-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', Themonic_TEXT_DOMAIN) . '</span></a>';
                echo '</li>';
    }
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="themonic-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', Themonic_TEXT_DOMAIN) . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . __('Import / Export Options', Themonic_TEXT_DOMAIN) . '</h3>';
                echo '<h4>' . __('Import Options', Themonic_TEXT_DOMAIN) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="themonic-opts-import-code-button" class="button-secondary">' . __('Import from file', Themonic_TEXT_DOMAIN) . '</a> <a href="javascript:void(0);" id="themonic-opts-import-link-button" class="button-secondary">' . __('Import from URL', Themonic_TEXT_DOMAIN) . '</a></p>';

                echo '<div id="themonic-opts-import-code-wrapper">';

                echo '<div class="themonic-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('themonic-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', Themonic_TEXT_DOMAIN)) . '</p>';
                echo '</div>';
                ...
                echo '<div class="themonic-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('themonic-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', Themonic_TEXT_DOMAIN)) . '</p>';
                echo '</div>';
                ...
                echo '<p id="themonic-opts-import-action"><input type="submit" id="themonic-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', Themonic_TEXT_DOMAIN) . '"> <span>' . apply_filters('themonic-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', Themonic_TEXT_DOMAIN)) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . __('Export Options', Themonic_TEXT_DOMAIN) . '</h4>';
                echo '<div class="themonic-opts-section-desc">';
                echo '<p class="description">' . apply_filters('themonic-opts-backup-description', __('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', Themonic_TEXT_DOMAIN)).'</p>';
                echo '</div>';

                echo '<p><a href="javascript:void(0);" id="themonic-opts-export-code-copy" class="button-secondary">' . __('Copy', Themonic_TEXT_DOMAIN) . '</a> <a href="'.add_query_arg(array('feed' => 'themonicopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="themonic-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="themonic-opts-export-link" class="button-secondary">' . __('Copy Link', Themonic_TEXT_DOMAIN) . '</a></p>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . __('Dev Mode Info', Themonic_TEXT_DOMAIN) . '</h3>';
                echo '<div class="themonic-opts-section-desc">';
			...
			echo '<a style="float:left; padding-right: 30px;" href="http://themonic.com" id="optionpanellogo" class="logo" target="_blank"><img src="'.$this->url.'img/logo2.png" /></a>';
