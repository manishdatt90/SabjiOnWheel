...
function removePro(proId)
{
	$.ajax
	 ({
			 ...
			 { 
				$("#proRow"+proId).remove();
               // alert(result.totalCartAmount);
				if(result.totalCartAmount!='0')
				{
				    $("#totCartAmt").html('Rs.'+parseFloat(result.totalCartAmount).toFixed(2));
				}
				else
				{
					$(".cart-items").remove();
					$("#emptyMsg").html('Empty Cart.Go to <a href="vegetables.php">Our Products</a>');
				}
             }
     });
}
